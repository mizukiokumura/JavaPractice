...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    // 練習問題 7-6;
      ...
      int CheckCoin = 0;
      for(int i = 0; i < 10; i++) {
        System.out.print("硬貨の種類は?(500,100,50,10,5,1)");
        int kind = Integer.parseInt(br.readLine());
        System.out.print("硬貨の枚数は?");
        int count = Integer.parseInt(br.readLine());
        Coin.GetCoins(kind, count);
      }
      System.out.println("どの硬貨の事を知りたいですか?終了したい場合0を入力してください");
      while(true) {
        System.out.println("どの硬貨の事を知りたいですか?(終了したい場合0を入力してください)");
        CheckCoin = Integer.parseInt(br.readLine());
        if(CheckCoin == 0) {
          break;
        } else {
          Coin.GetAmount(CheckCoin);
        }
      }
      Coin.GetCount();
